...
use crate::{app::App, format::truncate, qr};

/// State for the Addresses / receive view.
#[derive(Default)]
pub(crate) struct AddressesState {
    /// The selected account, as an index into the accounts list.
    pub(crate) account: usize,
    /// The selected address within the account's address list.
...
pub(crate) async fn on_key(app: &mut App, key: KeyEvent) {
    match key.code {
        ...
        KeyCode::Down | KeyCode::Char('j') => {
            ...
            if app.addresses.selected + 1 < n {
                app.addresses.selected += 1;
            }
        }
        KeyCode::Up | KeyCode::Char('k') => {
            app.addresses.selected = app.addresses.selected.saturating_sub(1);
        }
...
/// Copies the currently-selected address to the clipboard via OSC 52.
fn copy_selected_address(app: &mut App) {
    ...
    match crate::terminal::copy_to_clipboard(&entry.address) {
        Ok(()) => app.info(crate::fl!("tui-addr-copied")),
        Err(e) => app.error(crate::fl!("tui-addr-copy-failed", error = e.to_string())),
    }
}

fn select_next_account(app: &mut App) {
    if !app.data.accounts.is_empty() {
        app.addresses.account = (app.addresses.account + 1) % app.data.accounts.len();
    }
}
...
fn select_prev_account(app: &mut App) {
    if !app.data.accounts.is_empty() {
        ...
        app.addresses.account = (app.addresses.account + n - 1) % n;
    }
}

async fn derive_new_address(app: &mut App) {
    let Some(account) = app.data.accounts.get(app.addresses.account) else {
        app.error(crate::fl!("tui-addr-no-account-selected"));
        return;
    };
...
fn render_account_selector(app: &App, frame: &mut Frame<'_>, area: Rect) {
    let label = match app.data.accounts.get(app.addresses.account) {
...
fn render_list(app: &App, addresses: &[AddressEntry], frame: &mut Frame<'_>, area: Rect) {
    ...
    state.select(Some(app.addresses.selected.min(addresses.len() - 1)));
