...
use crate::app::{App, TX_PAGE_SIZE};
use crate::format::truncate;

/// State for the Transactions view (a paginated list).
#[derive(Default)]
pub(crate) struct TransactionsState {
    /// The selected row, as an index into the current page of transactions.
    pub(crate) selected: usize,
    /// The offset of the current page, in transactions, from the most recent.
...
pub(crate) async fn on_key(app: &mut App, key: KeyEvent) {
    match key.code {
        KeyCode::Down | KeyCode::Char('j') => {
            if app.transactions.selected + 1 < app.data.transactions.len() {
                app.transactions.selected += 1;
            }
        }
        KeyCode::Up | KeyCode::Char('k') => {
            app.transactions.selected = app.transactions.selected.saturating_sub(1);
        }
        KeyCode::Char(']') => {
            // Next page (only if the current page was full).
            if app.data.transactions.len() as u32 == TX_PAGE_SIZE {
                app.transactions.offset += TX_PAGE_SIZE;
                app.transactions.selected = 0;
                app.refresh_transactions().await;
            }
        }
        KeyCode::Char('[') => {
            if app.transactions.offset >= TX_PAGE_SIZE {
                app.transactions.offset -= TX_PAGE_SIZE;
            } else {
                app.transactions.offset = 0;
            }
            app.transactions.selected = 0;
            app.refresh_transactions().await;
        }
...
fn render_list(app: &App, frame: &mut Frame<'_>, area: Rect) {
    let page = app.transactions.offset / TX_PAGE_SIZE + 1;
    ...
    state.select(Some(
        app.transactions
            .selected
            .min(app.data.transactions.len() - 1),
    ));
...
fn render_detail(app: &App, frame: &mut Frame<'_>, area: Rect) {
    ...
    let lines: Vec<Line<'_>> = match app.data.transactions.get(app.transactions.selected) {
        Some(tx) => {
            let mut lines = vec![
...
fn format_zec(zat: u64) -> String {
    crate::fl!("tui-amount-zec", amount = crate::format::zec_decimal(zat))
}
